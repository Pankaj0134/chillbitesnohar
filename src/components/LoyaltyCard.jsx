...
export default function LoyaltyCard() {
  ...
  // that code up from the URL once on load. This only works because the
  // customer actually scanned a fresh QR; typing a remembered old code
  // here will still fail server-side once it's expired or used.
  useEffect(() => {
    ...
    if (code) {
      ...
      // copied from the address bar and reused later.
      params.delete("claim");
...
async function handleClaim() {
    ...
    if (!result.success) {
      ...
      setClaimCode(null); // a failed code is burned either way — don't let retry reuse it
      return;
    }
